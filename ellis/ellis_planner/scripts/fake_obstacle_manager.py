...
class FakeObstacleManager:
    ...
    def init(self):
        ...
        rospy.loginfo("[FakeObstacleManager] Waiting for serice {}...".format(
        ...
        if not self.spawn_obstacles():
            rospy.logwarn("[FakeObstacleManager] Failed to spawn some obstacles")
    ...
    def spawn_obstacles(self):
        for o in self._obstacles:
            ...
            try:
                ...
                if not res.success:
                    rospy.logerr(
                        "[FakeObstacleManager] Failed to spawn obstacle \"{}\"".
                        format(o['name']))
                    return False
            except rospy.ServiceException as e:
                rospy.logerr(
                    "[FakeObstacleManager] Failed to call spawn service: {}".
                    format(e))
                return False
    ...
    def model_states_callback(self, msg):
        for o in self._obstacles:
            ...
            if i < 0:
                rospy.logwarn(
                    "[FakeObstacleManager] Couldn't get state of \"{}\"".format(
                        o['name']))
                continue

            obstacle_msg = Marker()
            obstacle_msg.header.frame_id = self._world_frame_id
            obstacle_msg.ns = "/fake_obstacle_manager/" + o['name']
            obstacle_msg.id = 0
...
def main():
    rospy.init_node('fake_obstacle_manager')

    mgr = FakeObstacleManager()
    if not mgr.init():
