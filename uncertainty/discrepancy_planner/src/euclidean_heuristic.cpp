...
EuclideanHeuristic::EuclideanHeuristic(FreeFlyerStateSpace* state_space, double cost_per_meter)
...
EuclideanHeuristic::CostType EuclideanHeuristic::Value(State* state) {
  ...
  double x, y, z, unused;
  state_space_->GetPose(state->GetVariables(), x, y, z, unused, unused, unused, unused, unused);
  ...
  double dist_to_goal = std::sqrt(std::pow(goal_x - x, 2) + std::pow(goal_y - y, 2) + std::pow(goal_z - z, 2));
