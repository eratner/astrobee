 ...
 public:
  ...
  State<StateDim>* Search(State<StateDim>* start_state) {
    if (!start_state->GetSearchInfo()) {
      ...
      if (heuristic_) heur = heuristic_->Value(start_state);
    ...
    while (open.Size() > 0) {
      ...
      for (auto action : actions) {
        ...
        if (!succ->GetSearchInfo()) {
          ...
          typename StateSpace<StateDim>::CostType heur = 0.0;
          if (heuristic_) heur = heuristic_->Value(succ);
    ...
    if (update_heuristic_) UpdateHeuristic(closed, best_state);
 ...
 private:
  bool OpenState(OpenList& open, State<StateDim>* state) {
    ...
    if (!state->InPriorityQueue()) {
      ...
      if (!open.DecreasePriority(
            state, state_info->cost_to_come_ + state_info->heur_cost_to_go_)) {
