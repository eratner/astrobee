...
template <unsigned int Dim> class PolynomialTrajectory {
public:
  struct Segment {
    ...
    if (waypoints.size() > 1) {
      ...
      for (int i = 0; i < waypoints.size() - 1; ++i) {
        ...
        for (int j = 0; j < Dim; ++j) {
          ...
          boundary_values[1] = 0;
          if (i > 0)
            boundary_values[1] = end_velocities.back()[j];
  ...
  bool Get(double time, std::array<double, Dim> &waypoint,
           int deriv = 0) const {
    double start_time, end_time;
    if (!GetStartTime(start_time) || !GetEndTime(end_time))
      return false;
    ...
    for (int i = 0; i < Dim; ++i) {
      double value;
      if (!segment.Get(time, i, value, deriv))
        return false;
  ...
  PolynomialTrajectory Subtraj(double start_time, double end_time) const {
    ...
    if (start_index < 0) {
      PRINT_WARN_STREAM("[PolynomialTrajectory::Subtraj] Could not find "
